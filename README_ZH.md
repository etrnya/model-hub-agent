...
1. **第一階段：基礎架構覺醒 (核心適配器與防禦)**
   - **起因**：初期發現 AI 代理在執行複雜 JSON 輸出任務時，API 請求極易因模型輸出微小語法錯誤而報錯重試，且冗長的對話歷史容易導致模型忘記最初的 objective。
...
3. **第三階段：後付款 Vertex AI 整合 (繞過 AI Studio 429 Prepay Bug)**
...
4. **第四階段：禁止瀏覽器 ODM 濫用 (限制高 Token 消耗工具)**
...
5. **第五階段：文件預處理轉 Markdown (MarkItDown 整合)**
